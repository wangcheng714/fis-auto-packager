...
{%$wdata = $widget_data.data|f_escape_xml%}
{%$tuan = $wdata.tuan|f_escape_xml%}
{%$premium = $wdata.ext.detail_info.premium2|f_escape_xml%}
<div class="place-widget-promotionm">
{%if !empty($tuan.groupon)%}
{%$point = $data.content.ext.detail_info.point|f_escape_xml%}
...
{%/if%}
{%if !empty($premium)%}
<a href="{%$widget_data.data.ext.detail_info.promotion_uri|cat:'&detail_part=premium&item_index=0'|f_escape_xml%}" data-log="{code:{%$STAT_CODE.PLACE_DETAIL_PREMIUM_CLICK|f_escape_xml%}, wd:'{%$widget_data.src_name|f_escape_xml%}', name:'{%$widget_data.name|f_escape_xml%}'}">
{%if !empty($tuan.groupon)%}
