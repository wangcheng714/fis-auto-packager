define('place:widget/order/order.js', function(require, exports, module){
...
module.exports = {
    ERRORMSG : {
        ...
        REPEATERROR: '系统不支持重复预定',
        CONTACTNAMEERROR: '身份证姓名请输入两个以上汉字',
        CONTACTIDERROR: '请输入有效的身份证号',
        REPEATPHONE: '手机号已经被其他身份证号注册，请更换',
    ...
    bind : function () {
        var self = this;
        this.wrapper.find('select').on('change', function(e) {
            self.changeSelect(e);
        });
    ...
    **/
    ...
    renderRoom : function(data){
        ...
        for(var i=1; i<=maxroom;i++){
            ...
            this.wrapper.find('select').append(temp.join(''));
        }
        this.totalPrice.attr('single',roomprice).html(roomprice);
    },
    /**
     * 房间数量选择
    ...
    getAllData : function(price){
        ...
        data.prices = this.totalPrice.attr('room_price')?this.totalPrice.attr('room_price'): self.getPrices(price);
    ...
    validateData :function(data){
        var phone = data.phone,
            ...
            contactId = data.contact_identity,
            verifycode =data.code,
            reg = /^[\u4E00-\u9FA5]+$/, //全都是汉字
            reg1 = /^[A-Za-z]+$/,//全都是英文,
            regName = /^[\u0391-\uFFE5]{2,}$/,
            identityValidate = require('place:static/js/identityValidate.js'),
            needIdentity = parseInt(this.wrapper.find('input[name=src_need_identity]').val()),
            flag = true;
        ...
        }else if(needIdentity && !regName.test(contactName)) {
            popup.open({
                'text': this.ERRORMSG.CONTACTNAMEERROR,
                'autoCloseTime': 1500
            });
        }else if(needIdentity && !identityValidate.validate(contactId)) {
            popup.open({
                'text': this.ERRORMSG.CONTACTIDERROR,
                'autoCloseTime': 1500
            });
        }
    ...
    handleOrderSubmit :function(orderdata){  
        ...
        var self  = this,
            hotel_price = this.totalPrice.attr('single'),
            kehuduan = this.kehuduan,
            from = kehuduan ? 'webview' : 'maponline',
            pagestate = kehuduan ? '/kehuduan=1' : '',
            href = '/detail?qt=order&from='+from;
        $.ajax({
            type: 'POST',
            url: href,
            //url:"/order",
            ...
            success: function(result){
                ...
                switch(result.errorNo) {
                    case 0:
                        url.navigate( '/mobile/webapp/place/order/qt=thirdsrc_order_success&order_id='+result.order_id + pagestate);
                        break;
                    case 10004:
                        self.addLayer();
                    ...
                    case 30006:
                        self.addLayer();
                        popup.open({'text': self.ERRORMSG.REPEATERROR,'autoCloseTime':false,'isTouchHide':true});
                        break;
                    case 40003:
                        self.addLayer();
                        popup.open({'text': self.ERRORMSG.REPEATPHONE,'autoCloseTime':false,'isTouchHide':true});
                        break;
                    ...
                    default:
                        ...
                        switch(errNo){
                            ...
                            default:
                            //跳转错误页面
                                url.navigate( '/mobile/webapp/place/order/qt=hotel_error&msg='+errMsg + pagestate);
                                break;                                
                        }
                        break;
                }
    ...
    HandelRoomChange : function(price,number){
        var value,
            $title = this.wrapper.find('.number');
            $title.html(number+'间');
        this.wrapper.find('input[name=room_num]').val(number);
        ...
        this.totalPrice.html(price*number);
    },
    /**
     * 房间价格变化处理
    ...
    HandelPriceChange : function(price){
        var room_price = this.getPrices(price),
            ...
            roomNumber = this.wrapper.find('input[name=room_num]').val();
        this.totalPrice.html(single_price*roomNumber);
        this.totalPrice.attr('single',single_price);
        this.totalPrice.attr('room_price',room_price);
    }
}
