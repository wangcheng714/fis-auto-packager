<div class="place-widget-mcodeskey">
	...
	<section>
		...
		<div class="codes_msg layoutbox">
			<p>恭喜您，您已使用<span class="fback">{%$data.phone%}</span>的手机号成功在百度地图购买了<span class="fred">{%$data.num%}</span>张兑换码</p>
			{%foreach $list as $item%}
				{%if $item.is_used==2%}
					<p class="fgray">{%$item.cdkey%}</p>
				{%else%}
					<p class="fred">{%$item.cdkey%}</p>	
				{%/if%}
			{%/foreach%}
			{%if $data.res_num == 0%}
		...
		<div class="codes_exchange layoutbox">
			...
			{%if $data.day != 0%}
				<p class="text_center">离可兑换时间还有：<span class="day">{%$data.day%}</span>天</p>
			{%/if%}
	...
	{%script type="text/javascript"%}
        require('mcodeskey.js').initialize();
    {%/script%}
    <div class="hide">
        <script type="text/javascript">
