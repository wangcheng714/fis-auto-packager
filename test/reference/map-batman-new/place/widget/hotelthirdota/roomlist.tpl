...
<div class="room-list">
    {%section name=rooms loop=$widget_data.room_info%}
        {%if $smarty.section.rooms.index<3%}
            <div class="room-item" ota_price_url="{%$widget_data.room_info[rooms].ota_price_url%}" index="{%$smarty.section.rooms.index%}">
        {%else%}
            <div class="room-item hide" ota_price_url="{%$widget_data.room_info[rooms].ota_price_url%}" index="{%$smarty.section.rooms.index%}">
        {%/if%}
                <span class="arrow-icon{%if $smarty.section.rooms.index==0 %} arrow-icon-open{%/if%}"></span>
                <span class="room-type" style="max-width: -webkit-calc(100% - 150px); max-width: calc(100% - 150px);">{%$widget_data.room_info[rooms].room_type_name%}</span>
                {%if $widget_data.room_info[rooms].lowest_price!=0 %}
                    <span class="room-price">
                        <span class="price-symbol">¥</span>{%$widget_data.room_info[rooms].lowest_price%}
                    </span>
