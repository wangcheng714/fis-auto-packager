...
<div class="place-widget-promotionm">
    ...
    {%$point = $data.content.ext.detail_info.point%}
        <a href="{%$tuan.groupon[0].groupon_webapp_url%}&uid={%$data.content.uid%}&from=map_webapp_detail&loc=({%$point.x%},{%$point.y%})" data-log="{code:{%$STAT_CODE.PLACE_DETAIL_GROUPON_CLICK%}, wd:'{%$widget_data.src_name%}', name:'{%$widget_data.name%}'}">
		{%if !empty($premium)%}
		...
		{%/if%}
                ...
                <ul class="groupon_detail">
                    <li class="stitle">{%$tuan.groupon[0].groupon_short_title%}</li>
                    <li class="title">{%$tuan.groupon[0].groupon_title%}</li>
                </ul>
                <div class="groupon_price">
                    <div class="price">
                        <div style="text-align:right;">
                            <span>￥</span><span>{%$tuan.groupon[0].groupon_price%}元</span>
                        </div>
                    </div>
                </div>
                <div class="clear"></div>
    ...
    {%/if%}
    {%if !empty($premium)%}
        <a href="{%$widget_data.data.ext.detail_info.promotion_uri|cat:'&detail_part=premium&item_index=0'%}" data-log="{code:{%$STAT_CODE.PLACE_DETAIL_PREMIUM_CLICK%}, wd:'{%$widget_data.src_name%}', name:'{%$widget_data.name%}'}">
            {%if !empty($tuan.groupon)%}
