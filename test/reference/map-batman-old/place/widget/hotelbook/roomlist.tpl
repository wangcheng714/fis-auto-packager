    ...
    <ul class="room-list">
        {%section name=rooms loop=$widget_data.room_info%}
            {%if $smarty.section.rooms.index<3%}
                <li class="room-item" ota_price_url="{%$widget_data.room_info[rooms].ota_price_url%}" index="{%$smarty.section.rooms.index%}">
            {%else%}
                <li class="room-item hide" ota_price_url="{%$widget_data.room_info[rooms].ota_price_url%}" index="{%$smarty.section.rooms.index%}">
            {%/if%}
