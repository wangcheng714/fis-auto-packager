...
<div class="pricetotal"><strong>订单金额：&yen;<span id="priceTotal">{%$data.room.single_price|f_escape_xml%}</span></strong>(到店支付)</div>
...
{%if $data.hotel_info.reg_hint%}<p class="register-info">{%$data.room_info.reg_hint|f_escape_xml%}</p>{%/if%}
</div>
{%script%}
