{%style id="/widget/thumbnail/thumbnail.inline.less"%}.index-widget-thumbnail {
  /*定位条*/

  /*我的位置Marker*/

}
.index-widget-thumbnail .thumb {
  padding: 0px 8px 8px 8px;
  border-bottom: 1px solid #cecece;
}
.index-widget-thumbnail .thumb-icon {
  background: url(http://s1.map.bdimg.com/mobile/static/common/ui/images/ui3/index_loc_3f1b643e.png) no-repeat 0 0;
  background-size: 28px 151px;
}
.index-widget-thumbnail .thumb-wrap {
  border: 1px solid #dadada;
  position: relative;
  height: 101px;
  background-color: #ffffff;
  padding: 2px;
}
.index-widget-thumbnail .thumb-link {
  display: block;
  height: 100%;
}
.index-widget-thumbnail .thumb-img {
  width: 100%;
  height: 100%;
}
.index-widget-thumbnail .map-btn {
  position: absolute;
  right: 0px;
  bottom: 0px;
  height: 31px;
  width: 31px;
  background-color: white;
}
.index-widget-thumbnail .map-btn b {
  display: inline-block;
  margin: 7px 0 0 7px;
  height: 17px;
  width: 17px;
  background-position: 0px -52px;
}
.index-widget-thumbnail .geo-btn {
  position: absolute;
  left: 6px;
  bottom: 6px;
  height: 36px;
  width: 36px;
  background-color: #f4f4f4;
  border: 1px solid #83837c;
  border-radius: 5px;
  -webkit-box-shadow: 1px 1px 1px #dddddd;
  display: none;
}
.index-widget-thumbnail .geo-btn b {
  display: inline-block;
  margin: 6px 0 0 7px;
  height: 23px;
  width: 23px;
  background-position: 0px 0px;
}
.index-widget-thumbnail .geo-fail {
  background-color: #f4f4f4;
}
.index-widget-thumbnail .geo-fail b {
  background-position: 0px 0px;
}
.index-widget-thumbnail .geo-btn.active {
  background-color: #eeeeee;
}
.index-widget-thumbnail .geo-btn.active b {
  background-position: 0px 0px;
}
.index-widget-thumbnail .view-map-btn {
  padding-top: 41px;
  color: #4197d3;
  font-size: 14px;
  margin: 0 auto;
}
.index-widget-thumbnail .locbar {
  position: relative;
  padding: 5px 13px 0px;
  top: -8px;
}
.index-widget-thumbnail .locbar-area {
  border: 1px solid #D4D4D4;
  background-color: #F9F9F9;
  height: 33px;
  line-height: 33px;
  padding-right: 48px;
  display: -webkit-box;
}
.index-widget-thumbnail .locbar-area .thumb-left {
  display: inline-block;
  width: 43px;
  height: 33px;
  border-right: 1px solid #D4D4D4;
}
.index-widget-thumbnail .locbar-area .thumb-icon {
  position: absolute;
  top: 11px;
  left: 20px;
  display: inline-block;
  width: 30px;
  height: 22px;
  background-position: 0 0;
}
.index-widget-thumbnail a {
  color: #6A97C3;
}
.index-widget-thumbnail .locbar .in-btn {
  position: absolute;
  top: 6px;
  right: 22px;
  display: inline-block;
  width: 45px;
  height: 33px;
  line-height: 33px;
  text-align: center;
  font-size: 13px;
}
.index-widget-thumbnail .locbar .active {
  background-color: #efeeee;
}
.index-widget-thumbnail .locbar-txt {
  color: #A9A9A9;
  font-size: 14px;
  display: -webkit-flex;
  padding-left: 17px;
  position: absolute;
  line-height: 33px;
  overflow: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
  -webkit-box-flex: 1;
  width: 54%;
  top: 7px;
  left: 55px;
}
.index-widget-thumbnail .error-cnt {
  width: 100%;
  height: 100%;
  background-color: #f2f2f2;
  text-align: center;
  position: absolute;
  top: 0;
  left: 0;
  display: none;
}
.index-widget-thumbnail .center-marker {
  position: absolute;
  z-index: 200;
  -webkit-user-select: none;
  left: 50%;
  top: 50%;
}
.index-widget-thumbnail .center-marker div {
  position: absolute;
  width: 14px;
  height: 14px;
  background-image: url("data:image/png;base64,");
  background-repeat: no-repeat;
  -webkit-background-size: 13px 14px;
  top: -7px;
  left: -7px;
}
.index-widget-thumbnail .navi-marker div {
  background-image: url("data:image/png;base64,");
  -webkit-background-size: 30px 30px;
  background-size: 30px 30px;
  width: 30px;
  height: 30px;
  left: -15px;
  top: -15px;
}
{%/style%}<!-- @fileOverview 首页缩略图模板 by jican -->
<div class="index-widget-thumbnail">
    <!--<div class="thumb">
        <div class="thumb-wrap">
            <div class="mappic-loading page-loading slide in">
                <div>
                   <span class="dot-1"></span>
                   <span class="dot-2"></span>
                   <span class="dot-3"></span>
                   <span class="dot-4"></span>
                   <span class="dot-5"></span>
                   <span class="dot-6"></span>
                   <span class="dot-7"></span>
                   <span class="dot-8"></span>
              </div>
            </div>
            <a href="/mobile/webapp/index/index/foo=bar/vt=map&showmygeo=1" class="thumb-link" data-log="{code:{%$STAT_CODE.STAT_THUMB_IMG_CLICK%},state:'fail'}">
                <img class="thumb-img" src=""/>
            </a>
            <a href="/mobile/webapp/index/index/foo=bar/vt=map&showmygeo=1" data-log="{code:{%$STAT_CODE.STAT_THUMB_IMG_CLICK%}}" class="error-cnt" >
                <p class="view-map-btn">点击查看地图</p>
            </a>
            <div class="geo-btn"><b class="thumb-icon"></b></div>
             <a href="/mobile/webapp/index/index/foo=bar/vt=map&showmygeo=1" class="map-btn" data-log="{code:{%$STAT_CODE.STAT_THUMB_FULL_CLICK%}}"><b class="thumb-icon"></b></a>
            <div class="center-marker"><div></div></div>
        </div>
    </div> -->
    <div class="locbar">
        <div class="locbar-area">

            <span class="thumb-left"><em class="thumb-icon"></em></span>
            <span class="locbar-txt"></span>
        </div>
        <a class="in-btn" href="/mobile/webapp/index/setmylocation" data-log="{code:{%$STAT_CODE.STAT_THUNB_INPUTMYLOC%}}">修改</a>
    </div>
</div>

{%script%}
    (require("index:widget/thumbnail/thumbnail.js")).init();
{%/script%}