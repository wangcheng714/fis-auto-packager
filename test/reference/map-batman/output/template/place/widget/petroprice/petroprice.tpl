{%style id="/widget/petroprice/petroprice.inline.less"%}.place-widget-petroprice {
  overflow: hidden;
...
.petroprice-list .petroprice-item {
  display: -webkit-box;
  display: -webkit-flex;
  display: flex;
...
.petroprice-list .petroprice-item:last-child {
  border-right: none;
}
.petroprice-list .petroprice-item .name {
  display: block;
  background: #f6f6f6;
}
.petroprice-list .petroprice-item .price {
  display: block;
  background: #fff;
}
{%/style%}<!-- @fileOverview 油价详情 -->
{%* 参数：$widget_date *%}
