	...
	{%for $i = 0;$i < count($list) ;$i++%}
	   {%if ($i < 10)%}
		...
		<li class='exchange-gift-list1'>
			<div class='exchange-gift-img'><img src={%$list[$i].pic_url%}></div>
			<div class='exchange-gift-contain'>
				
			    <div class='exchange-gift-title'>{%$list[$i].title%}</div>
			    <div class='exchange-gift-des'>{%$list[$i].description%}</div>
                <div class='exchange-gift-all'>总量{%$list[$i].total_num%}件 剩余{%$list[$i].total_num-$list[$i].exchange_num%}件</div>
                <div class='exchange-gift-buy'>兑换（-{%$list[$i].need_integral%}积分）</div>
            </div>
		</li>
		{%$i=$i+1%}
		{%if ($i != count($list))%}
		<li class='exchange-gift-list2'>
			...
			<div class='exchange-gift-contain'>
			    <div class='exchange-gift-title'>{%$list[$i].title%}</div>
			    <div class='exchange-gift-des'>{%$list[$i].title%}</div>
                <div class='exchange-gift-all'>总量{%$list[$i].total_num%}件 剩余{%$list[$i].total_num-$list[$i].exchange_num%}件</div>
                <div class='exchange-gift-buy'>兑换（-{%$list[$i].need_integral%}积分）</div>
            </div>
		</li>
